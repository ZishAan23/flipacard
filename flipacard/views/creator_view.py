...
class creator_view:
    ...
    def __init__(self):
        ...
        self.fname_f = ft.TextField()
        self.save_dlg = ft.AlertDialog(title="Enter name of file ", content=self.fname_f, actions=[ft.TextButton("Save", on_click=self.final_save)])
    ...
    def final_save(self,e):
        self.save_dlg.open=False
        e.page.dialog = None
        ...
        pth = DECK_DIR / f"{self.fname_f.value}.fc"

        counter = 1
        while pth.exists():
            pth = DECK_DIR / f"{self.fname_f.value}_{counter}.fc"
            counter += 1

        print(pth)
        self.export_deck(pth)
    ...
    async def save_deck(self, e):
        ...
        add_to_overlay(e.page, self.save_dlg)
        self.save_dlg.open = True
        e.page.dialog=self.save_dlg
